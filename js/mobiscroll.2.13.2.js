        ...
        */
        that.show = function (prevAnim, prevFocus) {
            ...
            html = '<div lang="' + s.lang + '" class="mbsc-' + s.theme + ' dw-' + s.display + ' ' +
                ...
                (hasButtons ? '' : ' dw-nobtn') + '">' +
                    '<div class="dw-persp">' +
                        '<div class="dwo_bg"></div>' + // Overlay((isModal ? '<div class="dwo"></div>' : '') + )
    ...
    classes.Scroller = function (el, settings, inherit) {
        ...
        function generateWheelItems(i) {
            ...
            $.each(values, function (j, v) {
                ...
                html += '<div role="option" aria-selected="false" class="dw-li dw-v" data-val="' + keys[j] + '"' + (labels[j] ? ' aria-label="' + labels[j] + '"' : '') + ' style="height:' + itemHeight/50 + 'rem;line-height:' + itemHeight/50 + 'rem;">' +
        ...
        function scroll(t, index, val, time, active) {
            var rem = -val * itemHeight/50,
                style = t[0].style;

            if (rem == pixels[index] && iv[index]) {
                return;
            }
            ...
            if (has3d) {
                style[pr + 'Transform'] = 'translate3d(0,' + rem + 'rem,0)';
            } else {
                style.top = rem + 'rem';
            }
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    html += '<' + (hasFlex ? 'div' : 'td') + ' class="dwfl"' + ' style="' +
                                ...
                                '<div tabindex="0" aria-live="off" aria-label="' + lbl + '" role="listbox" class="dwww">' +
                                    '<div class="dww" style="height:' + (s.rows * itemHeight)/50 + 'rem;">' +
                                        '<div class="dw-ul" style="margin-top:' + (w.multiple ? 0 : s.rows / 2 * itemHeight - itemHeight / 2)/50 + 'rem;">';

                    // Create wheel values
                    html += generateWheelItems(l) +
                        '</div></div><div class="dwwo"></div></div><div class="dwwol"' +
                        (s.selectedLineHeight ? ' style="height:' + itemHeight/50 + 'rem;margin-top:-' + (itemHeight / 2 + (s.selectedLineBorder || 0))/50 + 'rem;"' : '') + '></div></div>' +
                        (hasFlex ? '</div>' : '</td>');
